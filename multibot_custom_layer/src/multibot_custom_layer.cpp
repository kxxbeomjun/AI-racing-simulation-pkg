#include<multibot_custom_layers/multibot_custom_layer.h>
#include <pluginlib/class_list_macros.h>
#include <predict_dyn_obs_path/Trajectory.h>

PLUGINLIB_EXPORT_CLASS(multibot_layer_namespace::MultibotCustomLayer, costmap_2d::Layer)

	using costmap_2d::LETHAL_OBSTACLE;

	namespace multibot_layer_namespace
{
	MultibotCustomLayer::MultibotCustomLayer() : resolution_(0)
	, inflation_radius_(0)
	, inscribed_radius_(0)
	, weight_(0)
	...
	void MultibotCustomLayer::onInitialize()
	{
		// ros::NodeHandle nh("~/" + name_);
		current_ = true;

		// Create a subscriber to the "/erp42_1/predict_trajectory" topic
		predict_traj_sub_ = nh.subscribe("/erp42_1/predicted_trajectory", 1, &MultibotCustomLayer::predictedTrajCallback, this);
		...
		ROS_INFO("on Initialize multibot custom layer");
		matchSize();
		ROS_INFO("Initialize multibot custom layer completed");
		}


	void MultibotCustomLayer::matchSize()
	{
		ROS_INFO("matchSize");
	...
	void MultibotCustomLayer::computeCaches()
	{
	...
	void MultibotCustomLayer::deleteKernels()
	{
	...
	void MultibotCustomLayer::reconfigureCB(costmap_2d::InflationPluginConfig &config, uint32_t level)
	{
	...
	void MultibotCustomLayer::predictedTrajCallback(const predict_dyn_obs_path::Trajectory::ConstPtr& msg)
	{
		const std::vector<double>& positions = msg->trajectory;

		double circle_radius = 8.0; // Set this to the desired radius of the circle

		// Reset the costmap layer before updating it with new costs
		costmap_2d::Costmap2D* costmap = layered_costmap_->getCostmap();

		// Iterate over the positions in the trajectory and compute costs for each cell along the way
		for (size_t i = 0; i < positions.size() / 2; i++) {
			double x = positions[2*i];
			double y = positions[2*i + 1];

			// Convert the (x, y) coordinates to map coordinates
			unsigned int mx, my;
			if (!layered_costmap_->getCostmap()->worldToMap(x, y, mx, my)) {
				continue;
			}

			// Iterate over all cells within the circle centered at (mx, my)
			for (int dx = -circle_radius; dx <= circle_radius; dx++) {
				for (int dy = -circle_radius; dy <= circle_radius; dy++) {
					int cx = mx + dx;
					int cy = my + dy;

					// Check if the cell is within the bounds of the costmap
					...
					double distance = hypot(dx, dy) * costmap->getResolution();
					// printf("distance: %f\n", distance);
					if (distance <= circle_radius) {
						// Set the cost for this cell in the costmap
						costmap->setCost(cx, cy, INSCRIBED_INFLATED_OBSTACLE - (distance * 5) - (i * 5));
						// printf("IIO: %d\n", INSCRIBED_INFLATED_OBSTACLE);
						// printf("dist x 5: %f\n", (distance * 5));
						// printf("i: %d\n", i);
						// printf("cost: %f\n", INSCRIBED_INFLATED_OBSTACLE - (distance * 5) - i);
					}
				}
			}
		}
	...
	void MultibotCustomLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y,
	...
	void MultibotCustomLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
			int max_j)
	{
		boost::unique_lock < boost::recursive_mutex > lock(*inflation_access_);
		...
		if (seen_ == NULL) {
			ROS_WARN("MultibotCustomLayer::updateCosts(): seen_ array is NULL");
			seen_size_ = size_x * size_y;
			seen_ = new bool[seen_size_];
		}
		else if (seen_size_ != size_x * size_y)
		{
			ROS_WARN("MultibotCustomLayer::updateCosts(): seen_ array size is wrong");
			delete[] seen_;
			seen_size_ = size_x * size_y;
			seen_ = new bool[seen_size_];
		}
	...
	void MultibotCustomLayer::onFootprintChanged()
	{
		...
		ROS_DEBUG("MultibotCustomLayer::onFootprintChanged(): num footprint points: %lu,"
					" inscribed_radius_ = %.3f, inflation_radius_ = %.3f",
					layered_costmap_->getFootprint().size(), inscribed_radius_, inflation_radius_);
	} 

	void MultibotCustomLayer::setInflationParameters(double inflation_radius, double cost_scaling_factor)
	{
	...
	inline void MultibotCustomLayer::enqueue(unsigned int index, unsigned int mx, unsigned int my,
