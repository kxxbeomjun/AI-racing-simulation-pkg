...
namespace regulated_pure_pursuit_controller
{
    ...
    void RegulatedPurePursuitController::initParams(ros::NodeHandle& nh){
        ...
        nh.param<bool>("use_collision_detection_constraints", use_collision_detection_constraints_, false);
                                                        ...
                                                        std::string &message)
    {
        ...
        getCmdVel_ = nh_.subscribe("cmd_vel", 1, &RegulatedPurePursuitController::getCmdVel, this);
        //ROS_INFO("Current CmdVel: %.2f   %.2f\n",CmdVel_.linear.x, CmdVel_.angular.z);
        double lookahead_dist = getLookAheadDistance(CmdVel_);
        ...
        const double & carrot_dist_const = std::hypot(carrot_pose.pose.position.x, carrot_pose.pose.position.y);
        double carrot_dist = carrot_dist_const;
        if(use_collision_detection_constraints_)
        {
            while(linear_vel > 0.3)
            {
    ...
    void RegulatedPurePursuitController::getRobotVel(geometry_msgs::Twist& speed){
        ...
        //ROS_INFO("Current linear.x: %.2f  angular.z: %.2f",speed.linear.x, speed.angular.z);
    }

    void RegulatedPurePursuitController::getCmdVel(const geometry_msgs::Twist& msg){
