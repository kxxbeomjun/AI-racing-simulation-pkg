...
def findLocalPath(ref_path, status_msg, path_frame, local_path_N): ## global_path와 차량의 status_msg를 이용해 현재waypoint와 local_path를 생성 ##
    ...
    if current_waypoint + local_path_length > len(ref_path.poses) :
...
class purePursuit : ## purePursuit 알고리즘 적용 ##
    ...
    def steering_angle(self): ## purePursuit 알고리즘을 이용한 Steering 계산 ## 
        ...
        if self.is_look_forward_point :
            ...
            rospy.loginfo("found forward : %.3f, %.3f, %i, %i, [Min/Current/Max/dis/vel] %.2f, %.2f, %.2f, %.2f, %.2f", \
                rotated_point.x, rotated_point.y, theta*180/pi, self.steering*180/pi, \
                self.min_lfd, self.lfd, self.max_lfd, self.dis, self.current_vel.x)
            return self.steering, self.forward_point, self.lfd ## Steering 반환 ##
        else : 
            rospy.logwarn("no found forward point : %.3f, %.3f, %i, %i, [Min/Current/Max/dis/vel] %.2f, %.2f, %.2f, %.2f, %.2f", \
                rotated_point.x, rotated_point.y, theta*180/pi, self.steering*180/pi, \
                self.min_lfd, self.lfd, self.max_lfd, self.dis, self.current_vel.x)
            return self.steering, self.forward_point, self.lfd
